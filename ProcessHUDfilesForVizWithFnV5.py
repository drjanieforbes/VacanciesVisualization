# ####################################################################
#
# Program:  ProcessHUDfilesForVizWithFnV5.py
#
...
# - updated to sort all the files (although only 3/2015 and forward need it)
# - updated to use the dbfread to read the original downloaded .dbf files
#
# This script was built with Python 3.6
#
# I'm using Anaconda for Python 3.6 for easy access to the rich number of
# available packages.
#
#   - use conda env list    # to list available environments
#   - activate py36         # activate py27
#   - idle                  # to start IDLE as the programming environment
#   - conda install pkg     # install a package (make sure py36 is activated)
#
# ####################################################################
# import libraries
# ####################################################################
...
def dbf2DF(dbfile, mycols):
    ...
    # sort the data frame (because some of the newer HUD files aren't sorted)
    pandasDFsorted = pandasDF.sort_values("GEOID",
                                        axis=0,         # sort by rows
                                        ascending=True,
                                        inplace=False,
                                        kind='quicksort',
...
This function is used to process the files in date order,
beginning with the earliest file (03/2008) up to the most recent
file.
...
def sortHUD(flist):
    ...
    for i in range(0,len(sortdFileNames)):
        ...
        # put the month/year back to the original MMYYYY
...
# Note:  record layouts are the same, column headings are the same
...
for myFile in fileNames:
    ...
    # I'm looking for the year specified within the filename:
    ...
    # get current month/year from the current filename
    ...
    for index, row in mypandasDF.iterrows():
        ...
        # get ready for next row, store off the State, County
